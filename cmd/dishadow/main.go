...
//
// It only runs read-only cases so far. Cases that change something need a copy of the
// fixture per service so the two are not fighting over the same objects; those are reported
// as skipped rather than run against a shared tree, because a difference produced by the
// harness itself is worse than no result.
package main
